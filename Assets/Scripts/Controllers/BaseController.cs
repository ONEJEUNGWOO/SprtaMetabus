...
public class BaseController : MonoBehaviour     //방 향변경 이동 등 동작을 처리하는 클래스, 값은 따로 받아오는 것 같다.
{
    ...
    [SerializeField] private Transform playerTrans;             //캐릭터 위치 받아서 다른 변수에 넣고 움직이는지 확인 할 변수
    ...
    protected virtual void Update()
    {
        ...
        float h = Input.GetAxisRaw("Horizontal");  // 여기서도 입력 따로 확인하기
        float v = Input.GetAxisRaw("Vertical");    

        bool isRun = h != 0 || v != 0;          // 하나라도 입력되면 true 아니면 false

        characterAnimator.SetBool("IsRun", isRun);    // 파라미터 연결해주기
    }
